...
def build_model(cfg):
    ...
    if model_type == "native_lite_v7":
        return NativeLiteDOANet(
            ...
            mamba_conv_kernel=getattr(m, "mamba_conv_kernel", 4),
    ...
    if model_type == "native_lite_v7_content_only":
        return NativeLiteContentOnlyDOANet(
            freq_bins=freq_bins,
            encoder_channels=getattr(m, "encoder_channels", [24, 40, 64]),
            encoder_out_dim=getattr(m, "encoder_out_dim", 96),
            encoder_variant=getattr(m, "encoder_variant", "v2_balanced"),
    ...
    if model_type == "native_lite_v7_early_fusion":
        return NativeLiteEarlyFusionDOANet(
            freq_bins=freq_bins,
            encoder_channels=getattr(m, "encoder_channels", [24, 40, 64]),
            encoder_out_dim=getattr(m, "encoder_out_dim", 96),
            encoder_variant=getattr(m, "encoder_variant", "v2_balanced"),
            early_fusion_dim=getattr(m, "early_fusion_dim", 80),
            gru_hidden_size=m.gru_hidden_size,
            gru_num_layers=m.gru_num_layers,
            temporal_encoder_type=getattr(m, "temporal_encoder_type", "gru"),
            mamba_num_layers=getattr(m, "mamba_num_layers", 2),
