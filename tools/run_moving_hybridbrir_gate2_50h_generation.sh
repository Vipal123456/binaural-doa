...
DATA_ROOT="$ROOT/data/librispeech_cipic_moving_hybridbrir_gate2_50h_v1"
LOG="$ROOT/outputs/logs_moving_hybridbrir_gate2_50h_v1_generation.log"
WORKER_LOG_DIR="$ROOT/outputs/logs_moving_hybridbrir_gate2_50h_v1_workers"

TRAIN_TOTAL=36000
EVAL_TOTAL=4500
TRAIN_JOBS="${TRAIN_JOBS:-8}"
EVAL_JOBS="${EVAL_JOBS:-3}"
...
{
  echo "[$(date '+%F %T')] moving hybrid BRIR gate2 50h parallel generation start"
  echo "data_root=$DATA_ROOT"
  echo "setting=train $TRAIN_TOTAL, val $EVAL_TOTAL, test $EVAL_TOTAL, duration=4s, label_steps=40"
  echo "hours=train 40h, val 5h, test 5h, total 50h"
...
root = Path("data/librispeech_cipic_moving_hybridbrir_gate2_50h_v1")
manifest = {
    "dataset": "librispeech_cipic_moving_hybridbrir_gate2_50h_v1",
    "hours": {"train": 40.0, "val": 5.0, "test": 5.0, "total": 50.0},
    "samples": {"train_subjects": 36000, "val_subjects": 4500, "test_subjects_unseen": 4500},
    "duration_sec": 4.0,
    "label_steps": 40,
    "label_source": "target",
    "condition": "moving_hybridbrir_gate2",
...
root = Path("data/librispeech_cipic_moving_hybridbrir_gate2_50h_v1")
expected_counts = {"train_subjects": 36000, "val_subjects": 4500, "test_subjects_unseen": 4500}
...
for split in ["train_subjects", "val_subjects", "test_subjects_unseen"]:
    ...
    for wp in wavs[: min(300, len(wavs))]:
    ...
    if file_ids:
        ...
        missing = sorted(expected_ids - set(file_ids))[:20]
    split_ok = (
        len(metas) == expected
        and len(wavs) == expected
        and duplicates == 0
        and not missing
        and set(wav_lengths) == {64000}
        and len(set(label_values)) == 72
        and bool(quality)
        and float(np.mean(quality)) == 1.0
    )
    all_ok = all_ok and split_ok
    print(f"[{split}] files metadata={len(metas)} wav={len(wavs)} expected={expected} sampled_wav_lengths={sorted(set(wav_lengths))} ok={split_ok}")
    print(f"[{split}] file_id_range={min(file_ids) if file_ids else 'n/a'}-{max(file_ids) if file_ids else 'n/a'} duplicates={duplicates} missing_first20={missing}")
...
print(f"[diagnostics] all_ok={all_ok}")
if not all_ok:
    raise SystemExit(2)
PY
  echo "[$(date '+%F %T')] moving hybrid BRIR gate2 50h parallel generation done"
} 2>&1 | tee "$LOG"
