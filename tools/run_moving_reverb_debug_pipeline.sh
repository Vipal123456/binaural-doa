...
DATA_ROOT="$ROOT/data/librispeech_cipic_moving_robust50h_v1/reverb_moving_debug"
CONFIG="$ROOT/configs/train_moving_reverb_debug_v7_dualcue_seq.yaml"
RUN_LOG="$ROOT/outputs/logs_moving_reverb_debug_pipeline.log"

cd "$ROOT"
mkdir -p "$ROOT/outputs"

{
  echo "[$(date '+%F %T')] moving reverb debug pipeline start"
...
import numpy as np
root = Path("data/librispeech_cipic_moving_robust50h_v1/reverb_moving_debug")
...
PY
  ...
  "$PYTHON" evaluate_moving.py --config "$CONFIG" \
    --checkpoint "$ROOT/outputs/checkpoints_moving_reverb_debug_v7_dualcue_seq/best.pth"
  echo "[$(date '+%F %T')] moving reverb debug pipeline done"
} 2>&1 | tee -a "$RUN_LOG"
