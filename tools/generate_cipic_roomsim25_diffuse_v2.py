#!/usr/bin/env python3
"""Generate the subject/room-disjoint CIPIC Roomsim25 diffuse-noise v2 dataset.
...
Train/validation examples are sampled independently for every subject-angle;
test examples are paired across the complete RT60 x SNR grid.
"""
...
TEST_NOISE_SCENES: Tuple[str, ...] = ("TBUS", "SPSQUARE", "NPARK")
...
def recipe(split: str, subject: str, angle: int, rt60_ms: int, distance_index: int,
    ...
    channel_b = DEMAND_CHANNELS[stable_int(seed, "noise_channel_b") % len(DEMAND_CHANNELS)]
    if channel_b == channel_a:
        channel_b = DEMAND_CHANNELS[(DEMAND_CHANNELS.index(channel_b) + 1) % len(DEMAND_CHANNELS)]
    return {
        "split": split,
        "subject_id": subject,
        "angle_deg": int(angle),
...
def validate_input_layout(args: argparse.Namespace) -> Dict[str, List[str]]:
    ...
    for scene in TRAIN_NOISE_SCENES + TEST_NOISE_SCENES:
...
def generate_diffuse_noise(row: Mapping[str, object], length: int, spacing_m: float) -> Tuple[np.ndarray, Dict[str, object]]:
    ...
    start_b = _noise_start(path_b, str(row["split"]), float(row["noise_u_b"]), sample_rate, source_length)
    # The two excitation intervals must not overlap in time, even when channels differ.
    if abs(start_b - start_a) < source_length:
        lo, hi = SPLIT_TIME_RANGES[str(row["split"])]
        start_b = _noise_start(path_b, str(row["split"]), (float(row["noise_u_a"]) + 0.5) % 1.0,
                               sample_rate, source_length)
    if abs(start_b - start_a) < source_length:
        raise RuntimeError(f"Unable to obtain non-overlapping ANF inputs: {path_a} {path_b}")
    ...
    if len(source_a) != source_length or len(source_b) != source_length:
        raise RuntimeError("Unexpected DEMAND segment length")
    # ANF assumes mutually independent inputs with the same power spectrum.
    # Two raw DEMAND excerpts violate the equal-spectrum assumption and produce
    # a noticeably biased output coherence. Estimate one shared scene spectrum,
    # then filter two independent Gaussian excitations with that spectrum.
    reference = np.concatenate([source_a - np.mean(source_a), source_b - np.mean(source_b)]).astype(np.float64)
    ...
    fir_taps = 513
    shaping_filter = firwin2(fir_taps, psd_freq / (sample_rate / 2.0), spectral_gain)
    rng = np.random.default_rng(stable_int(row["seed"], "anf_independent_excitation"))
    white = rng.standard_normal((2, source_length + fir_taps - 1))
    inputs = np.stack([
    ...
    outputs, _target, _matrix = generate_signals(inputs, params, decomposition="evd", processing="balance+smooth")
    ...
    return noise, {
        ...
        "noise_excitation": "independent_gaussian_DEMAND_PSD_shaped",
        "noise_shaping_fir_taps": fir_taps,
    }
...
def write_manifest(args: argparse.Namespace, output_root: Path, bundles: Mapping[str, Sequence[Mapping[str, object]]],
    ...
    manifest = {
        "name": "librispeech_cipic_roomsim25_diffuse_v2", "mode": args.mode,
        ...
        "noise_rendering": (
            "Habets ANF spherical diffuse field from two independent Gaussian excitations sharing a "
            "PSD estimated from two non-overlapping DEMAND excerpts"
        ),
        "anf": {"nfft": ANF_NFFT, "decomposition": "evd", "processing": "balance+smooth"},
