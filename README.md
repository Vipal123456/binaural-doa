# Cue-Factorized Binaural Statistics With Narrow- and Cross-Band Modeling for Target Speech Localization

## 1. 引言

双耳目标语音定位旨在从左右耳观测中估计目标语音的到达方向，是机器人听觉、智能交互和空间音频感知中的基础问题。耳间电平差（ILD）和耳间相位差（IPD）为水平面定位提供了主要空间信息，但在混响、多个定向干扰和扩散背景共同存在时，观测到的双耳关系会随时间和频率显著波动。局部时频单元可能由不同声源主导，仅依赖瞬时线索容易产生较大的方向偏差。

这类场景包含两个相互关联的问题。首先，ILD来自左右耳自功率之比，而IPD来自复互功率谱的相位，二者具有不同的统计定义和受污染方式，使用完全相同的时间聚合规则并不充分。其次，局部线索编码后若过早移除频率轴，模型将难以继续积累同一频带的长期轨迹，也无法检验相邻频带及全频范围内的方向一致性。

为此，本项目提出 **CF-NCBNet**（Cue-Factorized Narrow-/Cross-Band Network）。模型首先通过Cue-Factorized CPSD为ILD和IPD学习不同的短时统计权重，再使用LocalTF编码器同时生成逐帧空间向量 $\mathbf c_t$ 和有序空间网格 $\mathbf G^C$。共享频谱编码器相应地产生双耳频谱向量 $\mathbf m_t$ 和频谱网格 $\mathbf G^M$。随后，窄带/跨频模块在频率汇聚前依次建模同频长期轨迹、邻频关系和低秩全带关系，并以有界残差细化逐帧融合表示。当前主线包含212,103个可训练参数。

本文方法的重点包括：

1. 根据ILD和IPD不同的统计形成机制，设计线索专属的短时双耳统计，而不是对两类线索共享同一个加权估计。
2. 同时保留逐帧向量与有序频带网格，使局部空间估计、长期窄带轨迹和跨频关系在同一模型中具有明确分工。
3. 在多定向非语音干扰与DEMAND扩散背景共同存在的条件下，统一比较CF-NCBNet与FN-SSL、SDEL、DP-RTF和BiL等定位模型。

### 1.1 任务定义

输入是一段含混响目标语音、1--3个定向非语音干扰和可选扩散背景的双通道音频，模型输出目标语音在正面水平面的离散方位。对耳道 $e\in\{L,R\}$，双耳观测信号写为
...
$$
\Theta=\{-80^\circ,-65^\circ,-55^\circ,-45^\circ,-40^\circ, \ldots,40^\circ,45^\circ,55^\circ,65^\circ,80^\circ\}.
$$
...
$$
P_L^k=|X_L^k|^2,\qquad P_R^k=|X_R^k|^2,\qquad C^k=X_L^kX_R^{k*}.
$$
...
$$
r_I^k=-\frac14\min\left(|\rho^k-\bar\rho|,4\right), \qquad \rho^k=\log\frac{P_L^k+\epsilon}{P_R^k+\epsilon},
$$
...
$$
\Phi_L^I=\sum_k w_I^kP_L^k,\qquad \Phi_R^I=\sum_k w_I^kP_R^k,
$$

$$
\Phi_{LR}^P=\sum_k w_P^kC^k,\qquad \Phi_e^P=\sum_k w_P^kP_e^k.
$$

最终显式线索为

$$
\mathrm{ILD}=10\log_{10} \frac{\Phi_L^I+\epsilon}{\Phi_R^I+\epsilon},
$$

$$
\mathbf u_{\mathrm{IPD}}= \frac{\Phi_{LR}^P}{|\Phi_{LR}^P|+\epsilon} =\cos(\mathrm{IPD})+j\sin(\mathrm{IPD}),
$$

$$
\gamma= \frac{|\Phi_{LR}^P|} {\sqrt{\Phi_L^P\Phi_R^P+\epsilon}}.
$$
...
$$
[\widetilde{\mathrm{ILD}},\gamma], \qquad [\sin(\mathrm{IPD}),\cos(\mathrm{IPD}),\gamma].
$$

每个分支依次使用标准 $3\times3$ 卷积、depthwise $3\times3$ 卷积和 pointwise $1\times1$ 卷积，再将频率轴汇聚为 32 个有序频带。中间网格为

$$
\mathbf G^I,\mathbf G^P\in\mathbb R^{T\times32\times8}, \qquad \mathbf G^C=[\mathbf G^I;\mathbf G^P] \in\mathbb R^{T\times32\times16}.
$$

网格沿频带展平并经过轻量时间卷积，形成逐帧空间向量

$$
\mathbf c_t=[\mathbf c_t^I;\mathbf c_t^P]\in\mathbb R^{24}, \qquad \mathbf c_t^I\in\mathbb R^8,\quad \mathbf c_t^P\in\mathbb R^{16}.
$$
...
$$
\mathbf f_{M,t}=\tfrac12(\mathbf f_{L,t}+\mathbf f_{R,t}), \qquad \mathbf f_{D,t}=\mathbf f_{L,t}-\mathbf f_{R,t},
$$
...
NCBF 中的 BiGRU 处理保留频带轴的逐带序列；末端 BiGRU 处理已经完成频带汇聚的 104 维逐帧序列。两者输入和职责不同。

训练目标仅为标签平滑系数 $\eta=0.1$ 的 25 类交叉熵：

$$
\mathcal L=-\sum_{j=1}^{25}\widetilde y_j\log p_j, \qquad \widetilde y_j=(1-\eta)y_j+\frac{\eta}{25}.
$$
...
测试集位于 <code>data/librispeech_cipic_roomsim25_multidir_diffuse_test_v1/test</code>，用于评估多定向干扰与扩散背景同时存在时的目标语音定位性能。测试集包含300个基础声学场景；每个场景在36个严格配对条件下生成，总计10,800条双耳音频。

| 项目 | 设置 |
|---|---|
| 音频 | 16 kHz、2 秒、双通道 PCM16 |
| 目标 | LibriSpeech `test-clean` 语音，经 Roomsim-CIPIC BRIR 渲染 |
| 目标方位 | 25 个正面水平角；每个“条件 × 方位”包含 12 条音频 |
| 定向干扰 | 2 或 3 个经过自动人声过滤的 DNS wideband 非语音事件 |
| 主协议 | 聚合总 SIR 为 5、0、$-5$、$-10$ dB |
| 扩散条件 | 无扩散，或扩散 SNR 为 5、0、$-5$ dB |
| 强干扰协议 | 每个定向干扰独立设置为 $-5$ dB SIR，扩散 SNR 为 0 或 $-5$ dB |
| 混响 | 9 个 CIPIC subject、3 个房间，$T_{60}=0.2/0.6/0.8$ s |
| 空间约束 | 目标与干扰、干扰之间的最小角距均为 $20^\circ$ |

主协议由 $N\in\{2,3\}$、四档聚合总 SIR 和四档扩散条件组合成 32 个条件；强干扰协议包含 4 个条件。扩散背景采用六类 DEMAND 环境纹理（DKITCHEN、DWASHING、NFIELD、NRIVER、STRAFFIC 和 TCAR），并根据当前 CIPIC subject 的 HRTF 构造双耳空间相干性。所有条件共享相同的 300 个基础场景，且从 $N=2$ 到 $N=3$ 保持目标和前两个干扰源不变。
...
四项指标在全部 $M=10{,}800$ 条样本上整体计算。令第 $n$ 条音频的绝对角度误差为

$$
e_n=|\hat\theta_n-\theta_n|.
$$

MAE是所有样本角度误差的平均值；Acc@5和Acc@10分别表示误差不超过 $5^\circ$ 和 $10^\circ$ 的样本比例。LER45表示严重定位错误率（Large Error Rate at $45^\circ$）：

$$
\mathrm{LER45}=\frac{1}{M}\sum_{n=1}^{M}\mathbb I(e_n\geq45^\circ)\times100\%.
$$

LER45越低，说明模型产生大角度方向偏差的概率越小。它与MAE互为补充：MAE反映总体平均误差，LER45对少量但影响显著的灾难性定位错误更加敏感。每个模型报告当前已训练checkpoint中测试集MAE最低的一次完整运行，同行指标均来自同一个checkpoint，不跨运行拼接单项最优值。

| 模型 | MAE $\downarrow$ | Acc@5 $\uparrow$ | Acc@10 $\uparrow$ | LER45 $\downarrow$ |
|---|---:|---:|---:|---:|
| CF-NCBNet (Ours) | 6.628° | 75.60% | 86.30% | 2.68% |
| FN-SSL | 7.401° | 74.04% | 83.62% | 3.50% |
| SDEL | 8.469° | 71.82% | 82.24% | 4.46% |
| DP-RTF | 8.694° | 71.86% | 83.74% | 4.73% |
| BiL | 26.154° | 47.41% | 55.90% | 25.29% |
| B2（内部基线） | 9.887° | 70.49% | 80.23% | 6.22% |

CF-NCBNet取得最低的MAE和LER45。相对B2，CF-NCBNet的MAE从9.887°降至6.628°，同时将LER45从6.22%降至2.68%。
