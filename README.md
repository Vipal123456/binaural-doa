...
README 重点说明任务、方法和实验协议，暂不列出开发集或测试集性能。独立 <code>final-test</code> 尚未完成，因此仓库中的历史开发结果不应被解释为最终性能结论。
...
其中 $s$ 是唯一目标语音，$v_i$ 是第 $i$ 个定向非语音干扰，$d_e$ 是可选扩散背景。模型不估计干扰方位，也不使用目标说话人的注册语音或身份嵌入。因此，本项目属于**非语音事件干扰下的单目标语音定位**，不等同于目标说话人定位或多声源联合定位。
...
1. **线索专属短时统计**：ILD 由左右耳自功率比定义，IPD 由复互功率谱相位定义。CF-CPSD5 为二者学习不同的局部聚合权重。
...
- NCBF 和末端均使用双向 GRU，模型需要完整 2 秒片段，属于非因果模型。
...
softmax 前的分数裁剪至 $[-6,6]$。四个系数从零初始化，因此训练起点满足 $w_I^k=w_P^k=1/K$，即严格退化为均匀 CPSD5。
...
训练和验证划分在 LibriSpeech 说话人、DNS source ID、CIPIC subject 和模拟房间层面均不重叠。这些划分用于检查合成域内对未见说话人、事件、人头和房间的联合泛化，但不等价于真实录音评测。DNS 的人声排除是自动过滤结果，也不等价于逐文件人工确认。

<code>val_core</code> 是唯一用于 checkpoint 选择和早停的数据。<code>val_stress</code> 及历史 10,800 条复杂场景数据均已影响结构选择，只能视为开发集。模型、超参数和分析协议冻结后，还需要生成不参与开发的独立 <code>final-test</code>；在此之前不报告最终优越性结论。
...
- $K=5$ 来自开发集窗口消融；改变帧移、采样率或声学条件后需要重新验证。
- 257 个频点被自适应平均为 32 个均匀有序频带，压缩过程可能丢失细粒度频率结构。
- 双耳频谱支路的差分关系与显式 ILD 存在信息重叠，当前结构主要依靠实验而非严格的信息解耦保证其作用。
- 两级 BiGRU 分别处理逐带轨迹和频带汇聚后的帧序列，但都使用完整时间轴，仍存在可被质疑的功能重叠。
- 数据使用模拟 BRIR 和自动过滤的非语音事件，尚不能替代真实双耳录音、语音干扰或目标说话人选择实验。
- <code>final-test</code> 尚未锁定执行，因此 README 暂不提供实验结果表。
