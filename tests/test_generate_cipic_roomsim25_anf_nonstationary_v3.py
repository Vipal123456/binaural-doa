...
def test_r0_counts_are_exact():
    ...
    assert sum(row["rt60_ms"] == 0 for bundle in bundles["val"] for row in bundle["recipes"]) == 0
...
def test_anf_inputs_use_the_same_recording_channel():
    bundles = make_bundles("full", 42)
    for split in bundles.values():
        for bundle in split[:5]:
            assert all(row["noise_channel_a"] == row["noise_channel_b"] for row in bundle["recipes"])
